...
def run(
    ...
    segments = backends.diarizer.diarize(vocals_path, num_speakers=config.num_speakers)
    _validate_speaker_count(segments, config.num_speakers)
    ...
    log.info("Stage 5/8: transcribing (%d speaker segment(s))", len(segments))
    segments = backends.transcriber.transcribe(vocals_path, segments)
